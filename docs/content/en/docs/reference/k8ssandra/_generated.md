...
| cassandra.versionImageMap | object | `{"3.11.10":"datastax/cassandra-mgmtapi-3_11_10:v0.1.22","3.11.7":"datastax/cassandra-mgmtapi-3_11_7:v0.1.22","3.11.8":"datastax/cassandra-mgmtapi-3_11_8:v0.1.22","3.11.9":"datastax/cassandra-mgmtapi-3_11_9:v0.1.22","4.0.0":"datastax/cassandra-mgmtapi-4_0_0:v0.1.22"}` | Specifies the image to use for a particular Cassandra version. Exercise care and caution with changing these values! cass-operator is not designed to work with arbitrary Cassandra images. It expects the cassandra container to be running management-api images. If you do want to change one of these mappings, the new value should be a management-api image. |
...
| medusa.storage | string | `"s3"` | API interface used by the object store. Supported values include `s3`, 's3_compatible' and `gcs`. For pod mapped storage, use 'local' |
